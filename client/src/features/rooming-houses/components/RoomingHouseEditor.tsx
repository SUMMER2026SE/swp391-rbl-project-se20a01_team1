...
type RoomingHouseTab = "basic" | "images" | "amenities" | "legal";
...
function buildBasicForm(
  detail: RoomingHouseDetail | null,
): RoomingHouseBasicInfoRequest {
  if (!detail) {
    return emptyHouseForm;
  }
...
function buildLegalForm(
  detail: RoomingHouseDetail | null,
): UpdateLegalDocumentRequest {
  if (!detail) {
    return emptyLegalForm;
  }
...
export default function RoomingHouseEditor({
  ...
  const [roomingHouse, setRoomingHouse] =
    useState<RoomingHouseDetail | null>(initialRoomingHouse ?? null);
  const [activeTab, setActiveTab] = useState<RoomingHouseTab>("basic");
  ...
  const [basicForm, setBasicForm] =
    useState<RoomingHouseBasicInfoRequest>(() =>
  ...
  const [legalForm, setLegalForm] =
    useState<UpdateLegalDocumentRequest>(() =>
  ...
  async function saveLegalDocument() {
    ...
    if (!canEditLegalDocument) {
      setMessage(
        "Giấy tờ pháp lý chỉ được chỉnh sửa khi khu trọ là bản nháp hoặc bị từ chối.",
      );
      return;
    }
  ...
  async function uploadLegalImage(
    fieldName: LegalImageFieldName,
    file: File | null,
  ) {
    if (!file || !canEditLegalDocument) {
      return;
    }
    ...
    try {
      ...
      setLegalForm((current) => ({
        ...current,
        [fieldName]: uploaded.objectKey,
      }));
    } catch (error) {
      setMessage(
        getApiErrorMessage(
          error,
          "Không thể tải ảnh giấy tờ lên. Vui lòng kiểm tra định dạng và dung lượng ảnh.",
        ),
      );
    } finally {
      setSaving(false);
      ...
      <section className="rooming-house-editor__panel">
        ...
        {activeTab === "basic" && (
          <div className="rooming-house-editor__form-grid">
            <TextField label="Tên khu trọ" value={basicForm.name} onChange={(value) => setBasicForm({ ...basicForm, name: value })} />
            <TextField label="Mô tả" value={basicForm.description ?? ""} onChange={(value) => setBasicForm({ ...basicForm, description: value })} />
            <TextField label="Địa chỉ" value={basicForm.addressLine} onChange={(value) => setBasicForm({ ...basicForm, addressLine: value })} />
            ...
            <NumberField label="Vĩ độ" value={basicForm.latitude} onChange={(value) => setBasicForm({ ...basicForm, latitude: value })} />
            <NumberField label="Kinh độ" value={basicForm.longitude} onChange={(value) => setBasicForm({ ...basicForm, longitude: value })} />
            <ActionRow>
              <button className="rooming-house-editor__primary" onClick={saveBasicInfo}>
        ...
        {activeTab === "legal" && (
          <div className="rooming-house-editor__form-grid">
            ...
            <TextField readOnly={!canEditLegalDocument} label="Loại giấy tờ" value={legalForm.documentType} onChange={(value) => setLegalForm({ ...legalForm, documentType: value })} />
            <TextField readOnly={!canEditLegalDocument} label="Số giấy tờ" value={legalForm.documentNumber} onChange={(value) => setLegalForm({ ...legalForm, documentNumber: value })} />
            <LegalImageField
              label="Ảnh mặt trước"
            ...
            {canEditLegalDocument && (
              <ActionRow>
                <button className="rooming-house-editor__primary" onClick={saveLegalDocument}>
                  Lưu
                </button>
              </ActionRow>
            )}
